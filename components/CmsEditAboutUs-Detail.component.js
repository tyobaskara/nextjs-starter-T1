...
// Component
import CmsEditAboutUsDetailForm from '@components/CmsEditAboutUs-Detail-Form.component';
...
export default class CmsEditAboutUsDetail extends PureComponent {
  ...
  fetchDetailData = async () => {
    const { itemId, itemIndex, apiGetKey, apiGetDataUrl } = this.props;
    ...
    try {
      ...
      if (itemId) {
        detailData = response.data[apiGetKey][itemIndex];
      }
  ...
  _renderCmsEditAboutUsDetailForm = () => {
    const { sectionNumber, breadCrumbList, breadCrumbListActive, 
      apiUpdateUrl, inputFileList, itemId, contentType = '' } = this.props;
    ...
    const detailBreadCrumbList = [
      ...
      {
        route: '',
        name: name ? `${name}-${sectionNumber}` : sectionNumber
      }
    ];
    const updateUrl = itemId 
      ? `${apiUpdateUrl}?itemId=${itemId}`
      : apiUpdateUrl;

    return !isEmpty(detailData) && (
      <CmsEditAboutUsDetailForm 
