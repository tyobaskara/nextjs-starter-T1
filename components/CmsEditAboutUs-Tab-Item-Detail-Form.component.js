...
export default class CmsEditAboutUsDetailForm extends PureComponent {
  constructor(props) {
    super(props);
  ...
  _renderFormItems= () => {
    ...
    const formKeys = Object.keys(tabItemData);

    return this._getFormItems(tabItemData, formKeys);
  };

  _getFormItems = (tabItemData, keys) => {
    const _renderFormItems = [];

    for (let i = 0; i < keys.length; i++) {
      const topLevelLabelName = keys[i];
      const formItemData = tabItemData[topLevelLabelName];
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {!isEmpty(tabItemData) ? this._renderFormContainer() : null}
